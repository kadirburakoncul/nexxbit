...
public class UserStrategy : BaseEntity
{
    ...
    public string Timeframe { get; set; } = "15m";   // 5m gürültü + komisyon; 15m analizde tek kârlı bant
    ...
    public decimal VolatileMinChangePct { get; set; } = 5.0m;   // Minimum 24s % yükseliş filtresi
    ...
    public bool IsEma200RuleEnabled { get; set; } = true;    // uzun vadeli trend filtresi
    ...
    public bool IsVolumeSurgeFilterEnabled { get; set; } = true;
    public decimal VolumeSurgeMultiplier { get; set; } = 1.5m;

    // Piyasa rejimi filtresi (BTC EMA200'e göre bull/bear)
    public bool UseMarketRegimeFilter { get; set; } = true;  // bear piyasada alım yapma

    // Koruma ayarları
    public int MaxHoldHours { get; set; } = 24;   // 15m periyotta 8sa çok kısa
